...
# Precisa ter rodado o script "ModelToyv1.R"
#==========================================================================
...
# Damping ratio
# Quanto maior mais rápido o retorno ao ponto inicial
lapply(mpm_list_recruit_S11C,damping.ratio)%>%unlist()%>%mean()
lapply(mpm_list_recruit_S11C,damping.ratio)%>%unlist()%>%sd()

lapply(mpm_list_recruit_S11B,damping.ratio)%>%unlist()%>%mean()
lapply(mpm_list_recruit_S11B,damping.ratio)%>%unlist()%>%sd()

#----------------------------------------------------------------------------
#	Gráfico das taxas vitais
#----------------------------------------------------------------------------

apply(data.frame(do.call(rbind,lapply(mpm_list_fix1recruit,as.vector))),2,mean)
apply(data.frame(do.call(rbind,lapply(mpm_list_fix1recruit,as.vector))),2,sd)
...
projS11B<-stoch.projection(mpm_list_recruit_S11B, 
c(.5,.5),	#Estabelece meio indivíduo em cada estágio,logo, ao somar, teriamos quanto que a população doubrou,triplicou...
#	stable.stage(mean(mpm_list_recruit_S11B))*100,
	tmax=10,nreps=1000)

projS11C<-stoch.projection(mpm_list_recruit_S11C, 
c(.5,.5),	#Estabelece meio indivíduo em cada estágio,logo, ao somar, teriamos quanto que a população doubrou,triplicou...
#	stable.stage(mean(mpm_list_recruit_S11C))*100,
	tmax=10,nreps=1000)

apply(projS11C,1,sum)%>%range()
apply(projS11B,1,sum)%>%range()

prop.table(table(apply(projS11C,1,sum)>1)+
table(apply(projS11B,1,sum)>1))*100


rbind(
data.frame(projS11B,Site="S11B"),
data.frame(projS11C,Site="S11C"))%>%
