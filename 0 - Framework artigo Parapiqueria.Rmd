...
#Remove census in plot 15 in 2025 as it was extincted a year before
#Census_all #View
...
b <- c(0, 0, 1) # Creates the weight of each vital rate
```
...
-   Hypothetical population was created based on the matrix structure we expect for *Parapiqueria*. - Matrix is
    -   2x2
...
```{r}
lambsSite<-rbind(
do.call(rbind,lapply(mpm_list_S11C,lambda))%>%
	data.frame(Site="S11C")%>%
			rownames_to_column(var = "VAR"),
do.call(rbind,lapply(mpm_list_S11B,lambda))%>%
...
```{r message=FALSE, warning=FALSE} library(lme4) library(DHARMa) library(performance) library(glmmTMB) library(MuMIn)  options(na.action=na.fail) #Necessary for correct use of MuMIn}
```
...
data.frame(
  rbind(
    ...
    lapply(mpm_list,lambda)%>%unlist%>%mean),
    c(
    lapply(StochA,lambda)%>%unlist%>%sd,
    lapply(mpm_list,lambda)%>%unlist%>%sd)
    ),
...
lapply(mpm_list,lambda)%>%unlist%>%range)
```
