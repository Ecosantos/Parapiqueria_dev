...
-   Also, the transition of juveniles to adults and stasis $S_{1,1} + S_{2,1} ≤ 1$ cannot be greater than 1.
...
vr_plot<-vr.df%>%
  ...
  mutate(stages=case_when(vital_rate=="S1"~ "Stasis[Immat]",
                          vital_rate=="S2"~ "Maturation[Immat.>Adult]",
...
lamb_plot<-lamb_df%>%
  ...
  mutate(VAR="Population Growth rate")%>%
  ggplot(.,aes(y=Mean,x=Year,group=Site))+
...
#  facet_grid(.~VAR)+
ylab(expression("Population growth rate ("~lambda~")" ))+ xlab("Years")
...
lamb_plot+facet_wrap(.~VAR)+theme(legend.position="none")+xlab(NULL)+ylab(NULL))

ggsave(filename="Figures/Raw figures/Figure2.svg",height=8,width=15,dpi=600)
```
...
vr_perf%>%
  ...
  xlab(NULL)+ylab(expression("Proportonal contribution to "~lambda)),
vr_perf%>%
  pivot_longer(!c(vr, LTRE_sd),names_to="variable",values_to="values")%>%
    mutate(stages=case_when(vr=="S11"~ "Stasis[Immat]",
                          vr=="S21"~ "Maturation[Immat.>Adult]",
