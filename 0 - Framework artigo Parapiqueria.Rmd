...
nest(data = -stage)%>%
 mutate(
  ...
  unnest(tidy_fit)%>%mutate(across(where(is.numeric), ~round(.x, 4)))%>%
group_by(stage)%>%
select(-c(fit,data))%>%
group_split() # %>% map(~ as.data.frame(.x)) # To see other digits and solve a bug with tibble.
```
...
mutate(year=scale(as.numeric(year)))%>%   #Scale here is important to guarantee that the thousand is not important!
  glm(value~Site*year,data=.)%>%summary()
...
lamb_df%>%
mutate(Year=scale(as.numeric(Year)))%>%
glm(Lambda~Site*Year,data=.)%>%summary()
```
