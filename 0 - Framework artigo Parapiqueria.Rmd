...
2.  Estimate survival, stasis, and maturation using the `QPmat`from `popbio` package
...
#### Testing if there is differences in abundance between streams
...
nest(data = -stage)%>%
 mutate(
    AIC_neg.bin = map(data, ~ AIC(MASS::glm.nb(value ~ year*Site, data = .))),
    AIC_poisson = map(data, ~ AIC(glm(value ~ year*Site, family=poisson(),data = .)))
    )%>%
  unnest(AIC_neg.bin,AIC_poisson)%>%select(-data)
...
Census_all_max %>%
  ...
  nest(data = -stage) %>%
  mutate(
    fit = map(data, ~ MASS::glm.nb(value ~ Site*year, data = .)),
    anova = map(fit, ~ Anova(., type = 2)),
    tidy_anova = map(anova, ~ broom::tidy(.) %>%
                       mutate(across(where(is.numeric), round, 4)))
  ) %>%
  select(stage, tidy_anova) %>%
  group_by(stage)%>%
  unnest()%>%  group_split() # %>% map(~ as.data.frame(.x)) # To see other digits and solve a bug with tibble.
```

Plot abundance across years and streams
...
Census_all_max%>% 
  ...
  xlim(0,400)+
#scale_x_log10(breaks=c(1,10,100,370))+            
facet_grid(year~stage)+ 
  scale_color_manual(values=c("#ff9211","#6e86e7"))+ 
  scale_fill_manual(values=c("#ff9211","#6e86e7"))+ 
  theme_bw(base_size=24)+ 
  theme(legend.position="bottom")+ 
  labs(title=expression(paste("Abundance by plot of ", italic("Parapiqueria cavalcantei"))), 
       x="Maximum individuals recorded",
       y="Density")
```
...
digraph lifecycle {
  ...
  J -> A [label = <M<sub>1,2</sub>>, color = 'grey']
}
")
...
Non-zero elements in matrix $U$ are $S_{1,1}$ and $M_{2,1}$.

Once the matrix to represent our studied species' life cycle is defined, we can create the MPM for our hypothetical species. This is an important step to guide determine the necessary components to run the `QPmat` function
...
-   $S_{1,1} ≥ 0$ and $S_{1,1} ≥ 0$ means that components of the $U$ matrix cannot lower than 0.

-   Also, the transition of juveniles to adults and stasis $S_{1,1} + M_{2,1} ≤ 1$ cannot be greater than 1.
...
Define the weight of each demographic parameter. There is no reason to consider different weights in the demographic parameters we aim to estimate, so:
...
statrep(., group_vars = c("vital_rate"),value_var = "value",round = 4)
```
...
vr.df%>%
#filter(vital_rate=="R")%>%
  statrep(., group_vars = c("vital_rate","year", "Site"),value_var = "value")%>%
    group_by(Site)%>% group_split()
```
...
vr.df%>%
  ...
  mutate(year=as.factor(year))%>%   #Scale here is important to guarantee that the thousand is not important!
  glm(value~Site*year,data=.)%>%Anova(type = 2)
...
lamb_df%>%statrep(., group_vars = c("Year","Site"),value_var = "Lambda")%>%arrange(Site,desc(Year))%>%data.frame()
...
lamb_df%>%
mutate(Year=as.factor(Year))%>%
glm(Lambda~Site*Year,data=.)%>%Anova(type = 2)
```
...
vr.df%>%
  ...
  pull()%>%
  purrr::set_names(c("Stasis[Immat]", "Maturation[Immat.>Adult]", "Recruitment"))%>%
  tibble(
    Vital_rate = names(.),
...
vr_plot<-vr.df %>%
  ...
  mutate(stages = case_when(
    vital_rate == "S1" ~ "Stasis~(S[\"1,1\"])",
    vital_rate == "S2" ~ "Maturation~(M[\"2,1\"])",
    vital_rate == "R"  ~ "Recruitment (R)",
    vital_rate == "S4" ~ "Surv[Adult]"
  )) %>%
  mutate(stages = forcats::fct_relevel(stages, 
                                       c("Stasis~(S[\"1,1\"])", "Maturation~(M[\"2,1\"])", "Surv[Adult]", "Recruitment (R)"))) %>%
  filter(!(vital_rate %in% c("S4"))) %>%
  filter(!(vital_rate == "R" & year == 2025)) %>%
  ungroup() %>%
  ggplot(aes(y = Mean, x = year, group = Site)) +
  geom_pointrange(
    aes(ymin = Mean - SE, ymax = Mean + SE, color = Site, shape = year),
    alpha = 0.8,
    position = position_dodge2(width = 0.4), size = 1.2
  ) +
...
lamb_plot<-lamb_df%>%
  ...
  mutate(VAR="Population growth rate (λ)")%>%
  ggplot(.,aes(y=Mean,x=Year,group=Site))+
  ...
  geom_pointrange(aes(ymin=Mean-SE,ymax=Mean+SE,color=Site,shape=Year),alpha=0.8,
                  position = position_dodge2(width = 0.7),size=1.2)+
...
```{r, label="Figure2 Raw", eval=FALSE, include=FALSE, fig.height=8,fig.width=15}
p1<-p2<-p3<-vr_plot

p1$data<-vr_plot$data%>%filter(vital_rate=="S1")
p2$data<-vr_plot$data%>%filter(vital_rate=="S2")
p3$data<-vr_plot$data%>%filter(vital_rate=="R")



legend <- cowplot::get_legend(
  p1 + theme(legend.position = "bottom", legend.box.margin = margin(t = 10))
)
...
cowplot::plot_grid(
  ...
  cowplot::plot_grid(
    ncol=2,labels="AUTO",
    #Top
    p1+ theme(legend.position="none")+ylab("Transition probability"),
    p2+theme(legend.position="none")+ylab("Transition probability"),
    # Bottom
    p3+theme(legend.position="none")+ylab("Recruited indiv. per capita"),
    lamb_plot+facet_wrap(.~VAR)+theme(legend.position="none")+xlab(NULL)
  ),
  legend,
  ncol = 1,
  rel_heights = c(1, 0.1)  # Ajuste a altura da legenda se quiser
)%>%print()

ggsave(filename="Figures/Raw figures/Figure2.pdf",height=8,width=15,dpi=600)
```
...
cowplot::plot_grid(labels="AUTO",ncol=2,
vr_perf%>%
  pivot_longer(!c(vr, LTRE_sd),names_to="variable",values_to="values")%>%
    mutate(stages=case_when(vr=="S11"~ "Stasis[Immat]",
                          vr=="S21"~ "Maturation[Immat.>Adult]",
                          vr=="R"~ "Recruitment",
                          vr=="S22"~ "Surv[Adult]"))%>%
   mutate(stages=forcats::fct_relevel(stages, c("Stasis[Immat]","Maturation[Immat.>Adult]", "Surv[Adult]","Recruitment")))%>%
  filter(variable=="Elasticities")%>%
  filter(vr!="S22")%>%
  ggplot(.,aes(x=stages,y=values))+
  geom_col()+
  facet_grid(.~variable,scales="free")+
  theme_minimal(base_size=24)+
  theme(axis.text.x = element_text(angle = 75, hjust = 1))+  xlab(NULL)+ylab(expression("Proportonal contribution to "~lambda)),
vr_perf%>%
  pivot_longer(!c(vr, LTRE_sd),names_to="variable",values_to="values")%>%
    mutate(stages=case_when(vr=="S11"~ "Stasis[Immat]",
                          vr=="S21"~ "Maturation[Immat.>Adult]",
  ...
  xlab(NULL)+
  ylab(expression("Observed contribution to "~lambda )))
  

#ggsave(
#  filename = "Figures/Raw figures/figura 5.pdf",
#  width = 24,               
#  height = 16,              
#  units = "cm",
#  dpi = 300                 
...
-   Define the impacts to be simulated. a sequence of impact from 0% to 50% reduction in vital rates
...
# My color system - Define the colours for respective figures
Mycolors<-c(
...
ggsave(
  filename = "Figures/Raw figures/figura 6.pdf",
  width = 30,               
  height = 12,              
  units = "cm",
  dpi = 300                 
...
# 10. Data from Seed experiments and genetic diversity
```r
source("Seed experiment integration.R")
```
