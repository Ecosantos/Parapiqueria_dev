...
# ACHO QUE VALE A PENA JOGAR O RANDOM EFFECT PARA CONDITIONAL
# m4<-update(nb_RepIm,t1~0+t0+Site+period
# ASSIM CONSIGO ESTIMAR O EFEITO DA VARIAÇÃO AO LONGO DO ANO TBM.
# HÁ POUQUISSIMA MUDANÇA PRÁTICA NOS DEMAIS RESULTADOS!
...
ggeffects::predict_response(BestModel, terms=c("t0[0:208]","Site","period"))%>%plot()
...
prop.table(table(apply(projALL_10,1,sum)> n_threshold))*100
prop.table(table(apply(projALL_30,1,sum)> n_threshold))*100

apply(projS11C,1,sum)%>%range()
apply(projS11B,1,sum)%>%range()


ext10.probS11B<-table(apply(projS11B_10,1,sum)> n_threshold)
prop.table(ext10.probS11B)*100

ext10.probS11C<-table(apply(projS11C_10,1,sum)> n_threshold)
prop.table(ext10.probS11C)*100


prop.table(table(apply(projS11C_30,1,sum)> n_threshold))*100
prop.table(table(apply(projS11B_30,1,sum)> n_threshold))*100

ext10.probS11B
ext10.probS11C

result_table<- matrix(c(2, 998, 0, 1000), nrow = 2, byrow = TRUE)
result_table
